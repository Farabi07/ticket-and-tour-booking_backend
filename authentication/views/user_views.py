...
from authentication.serializers import (
    AdminUserSerializer, PasswordChangeSerializer, AdminUserListSerializer)
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        for k, v in serializer.items():
            data[k] = v
        data["ref_no"] = self.user.member.ref_no if self.user.member else None
        # data.pop('refresh')
        # data.pop('access')
        return data
...
def userHasPermission(request):
    ...
    try:
        ...
        response = {
            'detail': f"There is no such permission named '{permission_param}'."}
    ...
    if get_permission:
        ...
        response = {
            'detail': f"Pemission denied! this user has no '{permission_param}' permission."}
        return Response(response, status=status.HTTP_400_BAD_REQUEST)
...
def userPasswordChange(request, pk):
    try:
        ...
        if password == confirm_password:
            ...
            user.save()
            response = {
                'detail': f"User Id - {pk}'s password has been changed successfully."}
            return Response(response, status=status.HTTP_200_OK)
        else:
...
def checkUsernameWhenUpdate(request):
    ...
    if username is None:
        user_names_list = User.objects.filter(
            username=username).values_list('username', flat=True)
...
def checkEmailWhenUpdate(request):
    ...
    if email is not None:
        emails_list = User.objects.filter(
            email=email).values_list('email', flat=True)
...
def checkPrimaryPhoneWhenCreate(request):
    ...
    if _primary_phone is not None:
        user_objs = User.objects.filter(
            Q(primary_phone=primary_phone) | Q(secondary_phone=primary_phone))
...
def checkPrimaryPhoneWhenUpdate(request):
    ...
    primary_phones_list = []
    if _primary_phone is not None:
        _primary_phones_list = User.objects.filter(
            Q(primary_phone=primary_phone) | Q(secondary_phone=primary_phone)).values_list('primary_phone',
                                                                                           'secondary_phone')
...
def checkSecondaryPhoneWhenCreate(request):
    ...
    if _secondary_phone is not None:
        user_objs = User.objects.filter(
            Q(primary_phone=secondary_phone) | Q(secondary_phone=secondary_phone))
    else:
...
def checkSecondaryPhoneWhenUpdate(request):
    ...
    if _secondary_phone is not None:
        _secondary_phones_list = User.objects.filter(
            Q(primary_phone=secondary_phone) | Q(secondary_phone=secondary_phone)).values_list('primary_phone',
                                                                                               'secondary_phone')
...
@extend_schema(request=AdminUserSerializer, responses=AdminUserSerializer)
@api_view(['GET'])
def getUserWithoutId(request):
    try:
        user = request.user
        serializer = AdminUserSerializer(user)
        return Response(serializer.data)
    except ObjectDoesNotExist:
        return Response({'detail': f"User doesn't exists"})
